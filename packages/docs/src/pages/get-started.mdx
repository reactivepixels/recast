...
export const Button = ButtonPrimitive.recast({
  defaults: { variants: { intent: "primary", size: "md" } },
  base: {
    root: [
      "inline-flex",
      "items-center",
...
> **Note**: example above is using [Tailwind CSS](https://tailwindcss.com/) for styles but any css classes referencing a stylesheet or css modules could be used here instead. Please see [examples]() for more.
