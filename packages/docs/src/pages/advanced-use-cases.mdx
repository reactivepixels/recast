...
edge cases. For instance, in the example below, we are wrapping the
...
built-in orientation prop. In this scenario, we aim to spare the consumer from
having to add both the orientation and the variant prop, as the latter can be
inferred simply by the orientation prop.
